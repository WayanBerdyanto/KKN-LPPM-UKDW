@section('title', 'Laporan Kegiatan | Dosen')
...
@section('content')
                    ...
                    @foreach ($resultRencana as $idx => $item)
                        <tr class="bg-white border-b border-slate-200 hover:bg-slate-50 ">
                            ...
                            <td scope="row" class="px-2 py-4 max-w-20">
                                <a href="{{ route('dosendownloadlaporan', [$item->file]) }}"
                            ...
                            <td scope="row" class="px-2 py-4 max-w-36 truncate">
                                @if ($item->komentar_dosen != null)
                                    <button @click="modalUpdateKomentarLaporan = true" data-toggle="modal"
                                        data-target="#modalUpdateKomentarLaporan"
                                    ...
                                    <a href="#"
                                        onclick="confirmDelete('/dosen/deletekomentarlaporan/{{ $item->id_laporan }}')"
                                        class="bg-red-500 hover:bg-opacity-90 px-3 py-1 rounded-lg text-white content-center ml-2"
                                        data-confirm-delete="true">
                                        <i class="fa-solid fa-trash"></i>
                                    </a>
                                @else
                                    <button @click="modalKomentarLaporan = true" data-toggle="modal"
                                        data-target="#modalKomentarLaporan"
                                        class="bg-primary hover:bg-opacity-90 px-3 py-1 rounded-lg text-white content-center insertKomentarLaporan"
                                        data-id="{{ $item->id_laporan }}">
    ...
    <div x-show="modalUpdateKomentarLaporan" x-transition
        class="fixed left-0 top-0 flex h-full min-h-screen w-full items-center justify-center bg-dark/90 px-4 py-5 z-40">
        <div id="modalUpdateKomentarLaporan" @click.outside="modalUpdateKomentarLaporan = false"
            ...
            <form action="" method="POST">
                ...
                <div class="mb-4.5">
                    ...
                    <textarea name="komentar" cols="30" rows="8" id="komentar-laporan"
                ...
                <div class="-mx-3 flex flex-wrap">
                    <div class="w-1/2 px-3">
                        <button @click="modalUpdateKomentarLaporan = false" type="reset"
    ...
    <div x-show="modalKomentarLaporan" x-transition
        class="fixed left-0 top-0 flex h-full min-h-screen w-full items-center justify-center bg-dark/90 px-4 py-5 z-40">
        <div id="modalKomentarLaporan" @click.outside="modalKomentarLaporan = false"
            class="w-full max-w-[570px] rounded-[20px] bg-white px-8 py-12  dark:bg-dark-2 md:px-[70px] md:py-[60px]">
            <h3 class="pb-[18px] text-xl font-semibold text-dark text-center sm:text-2xl">
                Komentar Laporan Kegiatan
            </h3>
            ...
            <form action="" method="POST">
                ...
                <div class="-mx-3 flex flex-wrap">
                    <div class="w-1/2 px-3">
                        <button @click="modalKomentarLaporan = false" type="reset"
                            class="block w-full rounded-md border border-stroke p-3 text-center text-base font-medium text-dark transition hover:border-red-600 hover:bg-red-600 hover:text-white">
                            Cancel
                        </button>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.insertKomentarLaporan').click(function() {
                id_laporan = $(this).data('id');
                console.log(id_laporan);
                $('form').attr('action', '/dosen/laporan/postkomentarlaporan/' + id_laporan);
                $.ajax({
                    url: "/dosen/detaillaporan/" + id_laporan,
                    method: "GET",
                    dataType: "json"
                });
            });
                ...
                $.ajax({
                  url: "/dosen/detaillaporan/" + id_laporan,
                    ...
                    success: function(response) {
                        ...
                        $('#komentar-laporan').text(komentar);
                    },
