...
@section('content')
                ...
                <tbody>
                    @foreach ($resultRencana as $idx => $item)
                        <tr class="bg-white border-b border-slate-200 hover:bg-slate-50 ">
                            ...
                            <td scope="row" class="py-3 flex justify-center items-center">
                                <button @click="modaldetailrencana = true" data-toggle="modal"
                                    data-target="#modaldetailrencana"
                                    class="ml-0 lg:ml-5 mt-2 lg:mt-0 block bg-primary text-secondary px-1 py-2 font-normal rounded-lg text-center hover:bg-opacity-90 w-16">
                                    <i class="fa-solid fa-info"></i>&nbsp;
                                </button>
                                <button @click="modalupdatekegiatan = true" data-toggle="modal"
                                    data-target="#modalupdatekegiatan" data-id="{{ $item->id_rencana }}"
                                    class="ml-0 lg:ml-5 mt-2 lg:mt-0 block bg-primary text-secondary px-1 py-2 font-normal rounded-lg text-center hover:bg-opacity-90 w-16 updatekegiatan">
                                    <i class="fa -regular fa-pen-to-square"></i>&nbsp;
                                </button>
                                <button onclick="confirmDeleteRencana('/mahasiswa/deleterencana/{{ $item->id_rencana }}')"
    ...
    <div x-show="modalupdatekegiatan" x-transition
        class="fixed left-0 top-0 flex h-full min-h-screen w-full items-center justify-center bg-dark/90 px-4 py-5 z-40">
        <div id="modalupdatekegiatan" @click.outside="modalupdatekegiatan = false"
            class="w-full max-w-[570px] h-full min-h-full overflow-y-auto rounded-[20px] bg-white dark:bg-boxdark px-8 py-5  dark:bg-dark-2 md:px-[70px] md:py-[60px] border border-secondary">
            <h3 class="pb-[18px] text-xl font-semibold text-dark dark:text-secondary text-center sm:text-2xl">
            ...
            <form action="/mahasiswa/rencanakegiatan/postrencanakegiatan" method="POST" enctype="multipart/form-data">
                ...
                <div class="-mx-3 flex flex-wrap">
                    <div class="w-1/2 px-3">
                        <button @click="modalupdatekegiatan = false" type="reset"
                            class="block w-full rounded-md border border-stroke p-3 text-center text-base font-medium text-dark dark:text-secondary transition hover:border-red-600 hover:bg-red-600 hover:text-white">
                            Cancel
                        </button>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.updatekegiatan').click(function() {
                ...
                $('form').attr('action', '/mahasiswa/postUpdateLogbook/' + id_rencana);
                $.ajax({
                    url: "/mahasiswa/rencanakegiatan/detail/" + id_rencana,
                    method: "GET",
                    dataType: "json",
